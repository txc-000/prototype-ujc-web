...
import { 
    Users, FileText, BookOpen, Wallet, 
    ShieldCheck, PieChart, Building, LogOut, Code, Plane
} from 'lucide-react';
...
export default function DashboardSuperAdmin() {
    ...
    const modules = [
        ...
        // ── UPDATE ROUTING (Sesuai dengan perubahan App.jsx sebelumnya) ──
        { name: 'SPV Rekrutmen', path: '/supervisor/rekrutmen', icon: <ShieldCheck size={32} />, color: '#ec4899', desc: 'Master Data & Kontrol Alur Rekrutmen' },
        { name: 'SPV Dokumen', path: '/supervisor/dokumen', icon: <ShieldCheck size={32} />, color: '#eab308', desc: 'Approval & Kontrol Alur Dokumen' },
            ...
            <header style={styles.header}>
                <div>
                    ...
                    <p style={styles.subtitle}>
                        Akses sistem bypass: <span style={styles.highlight}>{userEmail}</span>
                    </p>
                </div>
...
// ── STYLE OBJECTS ──
const styles = {
    container: { minHeight: '100vh', background: '#0f172a', padding: '50px', fontFamily: 'sans-serif' },
    ...
    title: { color: 'white', margin: '0 0 10px 0', display: 'flex', alignItems: 'center', gap: '15px' },
    subtitle: { color: '#94a3b8', margin: 0, fontSize: '1.1rem' },
    highlight: { color: '#10b981', fontWeight: 800 },
    btnLogout: { background: '#ef4444', color: 'white', border: 'none', padding: '12px 24px', borderRadius: '8px', cursor: 'pointer', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '10px' },
    grid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '25px' },
    card: { background: '#1e293b', borderRadius: '12px', padding: '30px', cursor: 'pointer', transition: 'all 0.2s ease', display: 'flex', flexDirection: 'column', gap: '15px' },
    cardTitle: { color: 'white', margin: '0 0 5px 0', fontSize: '1.2rem', fontWeight: 800 },
    cardDesc: { color: '#94a3b8', margin: 0, fontSize: '0.9rem' },
    footer: { marginTop: '50px', textAlign: 'center', color: '#64748b', fontSize: '0.85rem' }
};