...
export default function DashboardSupervisor() {
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            if (data) {
                const getCount = (stage) => data.filter(s => cleanStr(s.tahap_sekarang) === cleanStr(stage) && isProses(s)).length;
    ...
    const generateChartData = (allData, chartType) => {
        ...
        if (chartType === 'ALL') {
            ...
            if (Object.keys(grouped).length === 0) setChartData([{ label: 'Belum Ada Data', value: 0, color: '#cbd5e1' }]);
            else setChartData(Object.keys(grouped).map(k => ({ label: `Bln: ${k}`, value: grouped[k], color: '#8b5cf6' })));
        }
    };
    ...
    const fetchMasterData = async () => {
        try {
            ...
            if (data) {
                ...
                else filteredData = data.filter(s => cleanStr(s.tahap_sekarang) === cleanStr(activeTab) && isProses(s));
    ...
    const handleNextStage = async (id, currentStage) => {
        const cleanCurrentStage = cleanStr(currentStage);
        const currentIndex = stagesOrder.findIndex(s => cleanStr(s) === cleanCurrentStage);
        
        if (currentIndex >= 0 && currentIndex < stagesOrder.length - 1) {
            const nextStage = stagesOrder[currentIndex + 1];
            if (!window.confirm(`Pindahkan siswa ke tahap: ${nextStage}?`)) return;
            const updates = { tahap_sekarang: nextStage, updated_at: new Date() };
            if (nextStage === 'Selesai') updates.status_akhir = 'Lulus';
            try { await supabase.from('students').update(updates).eq('id', id); fetchMasterData(); setActiveDropdown(null); } catch(err) { alert("Gagal memperbarui status."); }
        }
    };
                ...
                {activeMenu === 'DASHBOARD' && (
                    <div className="fade-in">
                        ...
                        <div style={{ background: 'white', padding: '30px', borderRadius: '12px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)', marginBottom: '30px' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '2px solid #f1f5f9', paddingBottom: '20px', marginBottom: '30px' }}>
                                ...
                                <div style={{ display: 'flex', gap: '8px', background: '#f8fafc', padding: '6px', borderRadius: '10px', border: '1px solid #e2e8f0', flexWrap: 'wrap' }}>
                                    ...
                                    {stagesOrder.filter(s=>s!=='Selesai').map(stage => (
                                        <button key={stage} onClick={() => setActiveChart(stage)} style={tabS(activeChart === stage)}>{stage}</button>
                                    ))}
                                </div>
                            </div>
                            
                            <div style={{ display: 'flex', alignItems: 'flex-end', gap: '30px', height: '280px', paddingBottom: '20px', paddingX: '20px' }}>
                                {chartData.map((data, i) => {
                                    ...
                                    return (
                                        <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', height: '100%', position: 'relative' }}>
                                            <div style={{ position: 'absolute', top: `calc(${100 - heightPct}% - 30px)`, background: '#1e293b', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '0.85rem', fontWeight: 800, transition: 'top 0.4s ease' }}>{data.value}</div>
                                            <div style={{ flex: 1, display: 'flex', alignItems: 'flex-end', width: '100%', padding: '0 15%' }}><div style={{ width: '100%', background: data.color, height: `${heightPct}%`, borderRadius: '6px 6px 0 0', transition: 'all 0.4s ease', opacity: data.value === 0 ? 0.3 : 1 }}></div></div>
                                            <div style={{ position: 'absolute', bottom: '-25px', width: '100%', textAlign: 'center', fontSize: '0.8rem', color: '#64748b', fontWeight: 700 }}>{data.label}</div>
                                        </div>
                                    )
                                })}
                            </div>
                        ...
                        <div style={{ background: 'white', padding: '30px', borderRadius: '12px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)' }}>
                            <h3 style={{ margin: '0 0 25px 0', color: '#1e293b', display: 'flex', alignItems: 'center', gap: '10px', fontWeight: 700 }}><Activity size={22} color="#10b981"/> Pipeline Conveyor</h3>
                            ...
                            <div>
                                ...
                                {activeMenu === 'MASTER_CV' && (
                                    <div style={{ display: 'flex', gap: '8px', background: '#e2e8f0', padding: '6px', borderRadius: '10px' }}>
                                        {['Pemberkasan', 'Keuangan', 'Pelatihan', 'Penempatan'].map(tab => (
