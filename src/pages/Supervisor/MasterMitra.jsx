...
export default function MasterMitra() {
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            const data = await supervisorService.getMitraList();
            setMitraList(data);
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Payload murni tanpa ID dan Password (dikirim terpisah)
            const basePayload = {
            ...
            if (editingId) {
                await supervisorService.updateMitra(editingId, basePayload);
                alert('Data Mitra berhasil diperbarui!');
            } else {
                if (!formData.password || formData.password.length < 6) {
                    throw new Error("Password minimal 6 karakter untuk akun Mitra.");
                }
                await supervisorService.addMitra(basePayload, formData.password);
    ...
    const handleDelete = async (id, nama) => {
        ...
        try {
            await supervisorService.deleteMitra(id);
    ...
    const handleViewStudents = async (mitra) => {
        ...
        setActiveDropdown(null); 
        try {
            const data = await supervisorService.getStudentsByMitra(mitra.nama_institusi);
    ...
    return (
        <div className="fade-in">
            <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
                ...
                <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
                    <div style={{ position: 'relative' }}>
                        ...
                        <input type="text" placeholder="Cari Institusi..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{ ...styles.inp, paddingLeft: '45px', width: '280px' }} />
                    </div>
                    ...
                    {filteredData.map(mitra => (
                        <div key={mitra.id} style={{ background: 'white', borderRadius: '12px', padding: '25px', boxShadow: '0 4px 10px rgba(0,0,0,0.04)', border: '1px solid #e2e8f0', position: 'relative', display: 'flex', flexDirection: 'column' }}>
                                ...
                                <div style={{ position: 'relative' }}>
                                    ...
                                    {activeDropdown === mitra.id && (
                                        <div ref={dropdownRef} style={styles.dropdownContainer}>
                                            <button onClick={() => handleViewStudents(mitra)} style={{...styles.dropdownItemS, color: brandNavy}}><Users size={14} /> Lihat Delegasi</button>
                                            <button onClick={() => openModal(mitra)} style={styles.dropdownItemS}><Edit size={14} /> Ubah Data</button>
                                            <button onClick={() => handleDelete(mitra.id, mitra.nama_institusi)} style={{ ...styles.dropdownItemS, color: '#ef4444' }}><Trash2 size={14} /> Hapus</button>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', flex: 1, marginTop: '10px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.85rem', color: '#64748b' }}><Phone size={14}/> <span>{mitra.no_telepon || '-'}</span> <span style={{color: '#cbd5e1'}}>•</span> <span>{mitra.penanggung_jawab || '-'}</span></div>
                                {mitra.email && <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.85rem', color: '#64748b' }}><Mail size={14}/> <span>{mitra.email}</span></div>}
                                {mitra.alamat && <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', fontSize: '0.85rem', color: '#64748b' }}><MapPin size={14} style={{marginTop:'3px', flexShrink: 0}}/> <span style={{lineHeight: 1.4}}>{mitra.alamat}</span></div>}
                            </div>
                                ...
                                <div style={styles.tableContainer}>
                                    <table style={styles.tableS}>
                                        <thead style={styles.theadS}>
                                            <tr>
                                                <th style={styles.thStyle}>Nama & NIK</th>
                                                <th style={styles.thStyle}>Program</th>
                                                <th style={styles.thStyle}>Posisi / Tahap Saat Ini</th>
                                                <th style={styles.thStyle}>Status MCU & Akhir</th>
                                            </tr>
                                        </thead>
                                        <tbody>
            ...
            {isModalOpen && (
                <div style={styles.modalOverlay}>
                    <form onSubmit={handleSubmit} style={{ ...styles.modalContent, width: '550px', padding: '30px' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', marginBottom: '25px', maxHeight: '65vh', overflowY: 'auto', paddingRight: '10px' }}>
                            ...
                            {!editingId && (
                                <div style={{ background: '#f8fafc', padding: '15px', borderRadius: '10px', border: '1px dashed #cbd5e1', marginBottom: '10px' }}>
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                                        <div><label style={styles.lb}>Email Login *</label><input type="email" name="email" value={formData.email} onChange={handleInputChange} required style={styles.inp} placeholder="email@sekolah.com" /></div>
                                        <div><label style={styles.lb}>Password Sementara *</label><input type="text" name="password" value={formData.password} onChange={handleInputChange} required style={styles.inp} placeholder="Min. 6 Karakter" /></div>
                                    </div>
                            ...
                            {editingId && (
                                <div><label style={styles.lb}>Email Kontak / Login</label><input type="email" name="email" value={formData.email} onChange={handleInputChange} required style={{...styles.inp, background: '#f1f5f9'}} readOnly title="Email login tidak bisa diubah dari sini." /></div>
                            )}

                            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '15px' }}>
                                <div><label style={styles.lb}>Nama Institusi / Sekolah *</label><input type="text" name="nama_institusi" value={formData.nama_institusi} onChange={handleInputChange} required style={styles.inp} placeholder="Contoh: SMKN 1 Jepara" /></div>
                                <div><label style={styles.lb}>Jenis</label>
                                    <select name="jenis_institusi" value={formData.jenis_institusi} onChange={handleInputChange} style={styles.inp}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                                <div><label style={styles.lb}>Penanggung Jawab (PIC)</label><input type="text" name="penanggung_jawab" value={formData.penanggung_jawab} onChange={handleInputChange} style={styles.inp} placeholder="Nama Guru / PIC" /></div>
                                <div><label style={styles.lb}>No. Telepon / WA</label><input type="text" name="no_telepon" value={formData.no_telepon} onChange={handleInputChange} style={styles.inp} /></div>
                            </div>
                            ...
                            <div><label style={styles.lb}>Alamat Lengkap</label><textarea name="alamat" value={formData.alamat} onChange={handleInputChange} rows="3" style={{ ...styles.inp, resize: 'vertical' }}></textarea></div>
                        </div>
